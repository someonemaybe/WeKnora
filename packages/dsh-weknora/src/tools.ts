 ...
 * for, and not already represented by a passage hit. ZS-HiAI's by-name search
...
export function createTools(client: WeknoraClient, config: ResolvedConfig): ToolDefinition[] {
  ...
  // An unscoped call reaches ZS-HiAI as a refusal on the retrieval route and as
  ...
  if (config.tools.listKnowledgeBases) {
    definitions.push({
      ...
      description: 'List the ZS-HiAI knowledge bases this deployment can retrieve from, with their ids. '
      ...
      output: {
        ...
        render: (_args, value) => {
          ...
          if (result.count === 0) return text('No knowledge base is available to this ZS-HiAI credential.')
  ...
  if (config.tools.search) {
    definitions.push({
      name: name('search'),
      description: 'Search ZS-HiAI knowledge bases and return the matching passages verbatim (hybrid vector + keyword '
      ...
      output: {
        ...
        render: (_args, value) => {
          ...
          if (result.count === 0) {
            if (named === '') {
              return text(`Nothing in ZS-HiAI matched "${result.query}" `
      ...
      async execute(args, exec): Promise<SearchValue> {
        ...
        // Fail here rather than let ZS-HiAI answer 400: the model can act on a
        // message naming the argument to supply, not on a transport error.
        if (knowledgeBaseIds.length === 0 && knowledgeIds.length === 0) {
          throw new Error(`${toolName}: this ZS-HiAI credential can see no knowledge base, so there is `
        ...
        const [hits, named] = await Promise.all([
          ...
          // By-name matching is an enrichment, and older ZS-HiAI builds may not
  ...
  if (config.tools.readDocument) {
    definitions.push({
      ...
      description: 'Read a ZS-HiAI document\'s stored passages in order, reassembled into text. '
  ...
  if (config.tools.ask) {
    ...
    definitions.push({
      name: name('ask'),
      description: `Ask ZS-HiAI a question and get its own composed answer with citations (${pipeline} pipeline runs `
      ...
      parameters: {
        ...
        properties: {
          ...
          session_id: { type: 'string', description: 'Continue an earlier ZS-HiAI session instead of starting one.' },
          web_search: { type: 'boolean', description: 'Let ZS-HiAI also search the web, when its deployment allows it.' },
        },
        required: ['query'],
        additionalProperties: false,
      },
      ...
      output: {
        ...
        render: (_args, value) => {
          ...
          parts.push(result.answer === ''
            ? 'ZS-HiAI returned an empty answer. Retry with a more specific question, or retrieve passages instead.'
            : result.answer)
          ...
          if (result.tool_calls.length > 0) parts.push(`ZS-HiAI tools used: ${result.tool_calls.join(', ')}`)
          parts.push(`ZS-HiAI session: ${result.session_id} (pass session_id to ask a follow-up in context)`)
