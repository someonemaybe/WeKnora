...
first-class access to a [ZS-HiAI](https://github.com/Tencent/WeKnora) knowledge base: hybrid retrieval, full document
reading, and ZS-HiAI's own composed answers with citations.
...
export WEKNORA_API_KEY=sk-...                          # a ZS-HiAI API key with `retrieve` (+ `chat` for ask)
...
- id: weknora
  config:
    ...
    knowledgeBaseIds:
      - kb-product-docs
    agentId: ''            # a ZS-HiAI custom agent id makes `weknora_ask` use the ReAct pipeline
    maxResults: 8
    maxChunkChars: 1200
...
| Tool | ZS-HiAI endpoint | What the model gets |
|---|---|---|
...
| `weknora_ask` | `POST /sessions` + `POST /knowledge-chat/:id` or `POST /agent-chat/:id` | ZS-HiAI's own answer, its citations, the server-side tools it used, and a resumable `session_id` |
...
`weknora_ask` delegates the whole question to ZS-HiAI. Reserve it for broad or synthesis questions spanning many
...
and ZS-HiAI's generated summary so a long document can be judged without paging through it.
...
The plugin only reads. It never writes to ZS-HiAI: no ingestion, no chunk edits, no deletion. A ZS-HiAI API key scoped
...
select a workspace. Errors are reported with the HTTP status and ZS-HiAI's own reason, never with the key.
...
With the default `resourceUrls: public`, ZS-HiAI rewrites cited `resource://` figures into directly loadable `http(s)`
URLs. Copy those Markdown images into the assistant reply and the dsh web UI will render the picture. A
knowledge-base-restricted API key cannot receive public URLs (ZS-HiAI answers 403); the plugin retries that call in
...
| `chatTimeoutMs` | `300000` | `weknora_ask`, which waits on ZS-HiAI's own model calls |
...
ZS-HiAI backend and a deterministic OpenAI-compatible model, and asserts that the harness really called the tools and
...
`test/fixtures/api-contract.json` records every ZS-HiAI call this plugin makes. `test/contract.test.mjs` asserts the
plugin still emits exactly those calls, and `contract/contract_test.go` asserts ZS-HiAI's real Go request and response
types still accept and serve them — so a rename on either side fails CI instead of a user's agent.

## Compatibility

Verified against dsh `0.1.0-rc.8` and ZS-HiAI `0.8.0`. dsh is in developer preview and states that breaking changes
