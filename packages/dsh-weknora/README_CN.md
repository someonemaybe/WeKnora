...
[ZS-HiAI](https://github.com/Tencent/WeKnora) 知识库：混合检索、按文档通读、以及 ZS-HiAI 自己带引用的成稿答案。
...
- id: weknora
  config:
    ...
    knowledgeBaseIds:
      - kb-product-docs
    agentId: ''            # 填 ZS-HiAI 自定义 Agent id，`weknora_ask` 就走 ReAct 流水线
    maxResults: 8
    maxChunkChars: 1200
...
| `weknora_ask` | `POST /sessions` + `POST /knowledge-chat/:id` 或 `POST /agent-chat/:id` | ZS-HiAI 自己的答案、引用、服务端用过的工具，以及可续聊的 `session_id` |
...
`weknora_ask` 把整个问题委派给 ZS-HiAI。它适合跨多篇文档、需要综合的宽泛问题，也就是自己检索要来回好几轮的场景；它在服务端
...
着这次调用需要的 `knowledge_id`；第一页会先给出文档标题和 ZS-HiAI 生成的摘要，长文档不必翻完才知道自己拿到的是什么。
...
插件只读。它不会写 ZS-HiAI：不入库、不改分块、不删除。四个工具里有三个只需要 `retrieve` 能力的 API Key；`weknora_ask`
额外需要 `chat`，因为它要创建会话并流式取回答案。

Key 通过 `X-API-Key` 发送。如果用的是平台级 API Key，还需要配 `tenantId`，它会作为 `X-Tenant-ID` 选择工作空间。报错信息
只包含 HTTP 状态码和 ZS-HiAI 自己给出的原因，不会带上 Key。

片段内容在进入模型前会按 `maxChunkChars` 截断，避免一份超大文档吃掉整个上下文窗口；截断这件事会明确告知模型
（`truncated: true`），而不是悄悄丢文本。

默认 `resourceUrls: public` 时，ZS-HiAI 会把片段里的 `resource://` 图改写成可直接加载的 http(s) 直链——把这些 Markdown
图片写进回复，dsh 网页端就会把图画出来。限定知识库的 API Key 拿不到直链（ZS-HiAI 会 403），插件会把这次调用改回
...
| `chatTimeoutMs` | `300000` | `weknora_ask`，要等 ZS-HiAI 自己的模型调用 |
...
端到端检查会把这个包装进一个临时 dsh profile，用 mock 的 ZS-HiAI 后端和一个确定性的 OpenAI 兼容模型启动 headless
...
`test/fixtures/api-contract.json` 记录了本插件发出的每一个 ZS-HiAI 调用。`test/contract.test.mjs` 断言插件仍然只发这些
调用，`contract/contract_test.go` 断言 ZS-HiAI 真实的 Go 请求/响应类型仍然接受并提供这些字段——任何一侧改名都会在 CI 挂掉，
而不是在用户的 Agent 里挂掉。

## 兼容性

已针对 dsh `0.1.0-rc.8` 与 ZS-HiAI `0.8.0` 验证。dsh 处于 developer preview 并明确会有破坏性变更；本包因此**没有任何运行
时依赖**，交给 `ctx.tools.register()` 的只是一个普通对象，不锁定任何 harness 包版本。如果 harness 的工具定义契约发生变化，
请在本仓库提 issue。

## 许可

MIT，与 ZS-HiAI 和 dsh 一致。
