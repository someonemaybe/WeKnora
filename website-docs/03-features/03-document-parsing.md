...
解析库是 Rust 静态库，需要 Rust 工具链构建。官方 Docker 镜像（`hiai/hiai-app`）和 `docker compose build` **默认链接** anydoc，设置页可直接选用。本地 `go build` 默认不链接：未加 `-tags anydoc` 时该引擎在「解析引擎」列表里显示为不可用，其它引擎不受影响。
...
```bash
docker build -f docker/Dockerfile.app --build-arg WITH_ANYDOC=0 -t hiai-app .
